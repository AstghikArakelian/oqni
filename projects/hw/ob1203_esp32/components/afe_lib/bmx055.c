...
void initBMX055()
{
   ...
   vTaskDelay(1000); // Wait for all registers to reset
...
//  Serial.println("Mag Calibration: Wave device in a figure eight until done!");
  vTaskDelay(4000);
   ...
   for(ii = 0; ii < sample_count; ii++) {
    ...
    vTaskDelay(105);  // at 10 Hz ODR, new mag data is available every 100 ms
   }
...
//	Wire.endTransmission();           // Send the Tx buffer
	...
	i2c_master_write_to_device(0, address, buffer, 2, 10);
}
...
//	return data;                             // Return data read from slave register
	...
	uint8_t count = 1;
	uint8_t data = 0;
//	i2c_master_write_read_device(0, address<< 1, subAddress, cmd_size, &data, count, 10);
//	return data;

uint16_t addr = address;

	i2c_master_write_to_device	(0, addr, &subAddress, cmd_size, 10);

	i2c_master_read_from_device (0, addr, &data, count, 10);
...
//        dest[i++] = Wire.read(); }         // Put read results in the Rx buffer
	...
	uint8_t cmd_size = 1;
	i2c_master_write_read_device(0, address, &subAddress, cmd_size, dest, count, 10);
}
