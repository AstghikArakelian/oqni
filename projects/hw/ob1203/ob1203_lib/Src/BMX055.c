...
extern I2C_HandleTypeDef* phi2c;
...
int BMX055_setup()
{
	...
	uint8_t acc_id = readByte(BMX055_ACC_ADDRESS, BMX055_ACC_WHOAMI);  		// Read ACC WHO_AM_I register for BMX055
	ob1203_Delay_ms(1000);
	uint8_t gyro_id = readByte(BMX055_GYRO_ADDRESS, BMX055_GYRO_WHOAMI);	// Read GYRO WHO_AM_I register for BMX055
	ob1203_Delay_ms(1000);
	writeByte(BMX055_MAG_ADDRESS, BMX055_MAG_PWR_CNTL1, 0x01); 				// wake up magnetometer first thing
	ob1203_Delay_ms(100);
	uint8_t mag_id = readByte(BMX055_MAG_ADDRESS, BMX055_MAG_WHOAMI);  		// Read MAG WHO_AM_I register for BMX055
	ob1203_Delay_ms(1000);

	if ((acc_id == 0xFA) && (gyro_id == 0x0F) && (mag_id == 0x32)) // WHO_AM_I should always be ACC = 0xFA, GYRO = 0x0F, MAG = 0x32
	{
		initBMX055();
		ob1203_Delay_ms(1000);
		// get sensor resolutions, only need to do this once
...
void initBMX055()
{
   ...
   ob1203_Delay_ms(1000); // Wait for all registers to reset
...
//  Serial.println("Mag Calibration: Wave device in a figure eight until done!");
  ob1203_Delay_ms(4000);
   ...
   for(ii = 0; ii < sample_count; ii++) {
    ...
    ob1203_Delay_ms(105);  // at 10 Hz ODR, new mag data is available every 100 ms
   }
...
//	Wire.endTransmission();           // Send the Tx buffer
	...
	ob1203_I2C_Write(address, buffer, 2);
}
...
//	uint16_t addr = address;
	...
	ob1203_I2C_Read(address, &subAddress, &data, cmd_size, count);
...
#if defined(HAL_STM32F103)
	uint8_t cmd_size = 1;
	HAL_I2C_Mem_Read(phi2c, address<< 1, subAddress, cmd_size, dest, count, 10);
#endif
}
